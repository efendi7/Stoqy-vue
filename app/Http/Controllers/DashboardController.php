...
class DashboardController extends Controller 
{
    ...
    public function index(Request $request)
    {
        ...
        if ($user->role === 'warehouse_manager') {
            ...
            $todayIncomingTransactions = StockTransaction::where('type', 'incoming')
                ->whereDate('transaction_date', Carbon::today())
                ->count();
            
            $todayOutgoingTransactions = StockTransaction::where('type', 'outgoing')
                ->whereDate('transaction_date', Carbon::today())
                ->count();

            // Pending transactions
            $pendingIncomingTasks = StockTransaction::with('supplier')
                ->where('type', 'incoming')
            ...
            $pendingOutgoingTasks = StockTransaction::where('type', 'outgoing')
                ->where('status', 'pending')
                ->latest()
                ->limit(5)
                ->get();
