...
<div class="container mx-auto px-4">
    ...
    {{-- Form Edit Transaksi --}}
    <form action="{{ route('stock_transactions.update', $transaction->id) }}" method="POST" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
        @csrf
        @method('PUT')
        ...
        <div class="mb-4">
            ...
            <input type="number" name="quantity" id="quantity" class="w-full px-3 py-2 border rounded shadow-sm focus:outline-none focus:ring focus:ring-blue-200" value="{{ old('quantity', $transaction->quantity) }}" required>
            @error('quantity')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-4">
            ...
            <input type="date" name="transaction_date" id="transaction_date" class="w-full px-3 py-2 border rounded shadow-sm focus:outline-none focus:ring focus:ring-blue-200" value="{{ old('transaction_date', $transaction->transaction_date) }}" required>
